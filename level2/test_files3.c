...
void test_file_open_invalid(void) {
    struct files_t* f = files_open("invalid.txt", 6);
    TEST_ASSERT_NULL(f);
...
void test_file_read1(void) {
    ...
    TEST_ASSERT_EQUAL_INT(2, f->ib_left);
...
void test_file_read2(void) {
    ...
    TEST_ASSERT_NOT_NULL(f->internal_buffer);
    TEST_ASSERT_EQUAL_INT(6, f->ib_size);
    TEST_ASSERT_EQUAL_INT(2, f->ib_position);
    TEST_ASSERT_EQUAL_INT(4, f->ib_left);
...
void test_file_read_all(void) {
    ...
    TEST_ASSERT_NOT_NULL(f->internal_buffer);
    TEST_ASSERT_EQUAL_INT(6, f->ib_size);
    TEST_ASSERT_EQUAL_INT(6, f->ib_position);
    TEST_ASSERT_EQUAL_INT(0, f->ib_left);
...
void test_file_open_large(void) {
    struct files_t* f = files_open("numbers_large.txt", 9);

    TEST_ASSERT_NOT_NULL(f);

    TEST_ASSERT_EQUAL_INT(0, f->position);
    TEST_ASSERT_NOT_NULL(f->internal_buffer);
    TEST_ASSERT_EQUAL_INT(9, f->ib_size);
    TEST_ASSERT_EQUAL_INT(0, f->ib_position);
    TEST_ASSERT_EQUAL_INT(0, f->ib_left);
...
void test_file_read_large_all(void) {
    ...
    TEST_ASSERT_NOT_NULL(f->internal_buffer);
    TEST_ASSERT_EQUAL_INT(9, f->ib_size);
    TEST_ASSERT_EQUAL_INT(1, f->ib_position);
    TEST_ASSERT_EQUAL_INT(0, f->ib_left);
